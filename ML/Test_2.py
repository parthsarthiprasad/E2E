...
class gaze_redirection_system:
    ...
    def flx_gaze(self, frame, gray, detections, shared_v, lock, pixel_cut=[3,4], size_I = [48,64]):
        ...
        cv2.imwrite(output_dir+str(self.img_count)+".jpg",frame)
    ...
    def run(self,shared_v,lock):
        ...
        while True:
            ...
            for img_name in items:
                self.counter+=1
                recv_frame = cv2.imread(input_dir+img_name)
                if(type(recv_frame)==type(None)):
                    continue
                recv_frame = cv2.resize(recv_frame,(size_window[0],size_window[1]))
                os.remove(input_dir+img_name)
                t = time.time()
